...
import { Difficulty, KnowledgeDomain, QuestionAttributes } from './model.js';
...
describe('regenerateQuestion', () => {
  it('uses the centralized prompt and the default model', async () => {
    mockedInvokeModel.mockResolvedValue('retried question');
    ...
    const input = mockedInvokeModel.mock.calls[0][0];
    expect(input.modelId).toBe(config.bedrockModelDefault);
    expect(input.text).toContain('knowledge domain Security');
    expect(input.text).toContain('topic IAM');
    expect(input.text).toContain(attributes.topicContext);
...
describe('generateQuestionRaw', () => {
  it('renders the centralized prompt and returns the raw response text', async () => {
    mockedInvokeModel.mockResolvedValue('raw question text');
    ...
    const input = mockedInvokeModel.mock.calls[0][0];
    expect(input.modelId).toBe(config.bedrockModelDefault);
    expect(input.text).toContain('Billing');
    expect(input.text).toContain('Pricing');
    expect(input.text).toContain(buildQuestionFormatSpec());
  });
...
describe('generateExamQuestions', () => {
  ...
  it('calls Bedrock once per question and returns raw responses', async () => {
    mockedInvokeModel.mockResolvedValue('raw response');
    ...
    expect(mockedInvokeModel).toHaveBeenCalledTimes(questionCount);
  });
  ...
  it('keeps responses aligned to question order when calls complete out of order', async () => {
    mockedInvokeModel.mockImplementation(async ({ text }) => {
      const match = text.match(/question number (\d+)/);
