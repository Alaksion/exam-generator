...
import {
  ...
  PlannerTopicGroup,
} from './conceptPlanner.js';
import { certification } from '../../test/fixtures/certification.js';
import { QuestionAttributes } from '../types.js';
...
function planResponseFor(numbers: number[]): { body: Uint8Array } {
  return makeBedrockResponse(
    JSON.stringify(numbers.map((number) => ({ number, concept: `concept-for-${number}` }))),
  );
}
...
describe('ConceptPlanSchema', () => {
  it('accepts a valid list of number/concept entries', () => {
    expect(ConceptPlanSchema.safeParse([{ number: 1, concept: 'lifecycle transitions' }]).success).toBe(
      true,
    );
  });
...
describe('planConcepts', () => {
  it('issues one planner call per topic group and returns the full 1:1 plan', async () => {
    sendMock.mockImplementation(async (command: { body: Buffer }) => {
      ...
      const match = parsed.messages[0].content[0].text.match(/Question numbers to cover: ([\d, ]+)/);
  ...
  it('runs topic groups within the concurrency pool limit', async () => {
    let inFlight = 0;
    let maxInFlight = 0;
    sendMock.mockImplementation(async (command: { body: Buffer }) => {
      inFlight += 1;
    ...
    await planConcepts(attributes, certification, 'corr-pool');
  ...
  it('retries a structurally mismatched plan and succeeds on a later attempt', async () => {
    ...
    sendMock.mockImplementation(async () => {
      call += 1;
      if (call <= 2) {
        return makeBedrockResponse(JSON.stringify([{ number: 999, concept: 'wrong' }]));
      }
  ...
  it('hard-fails after the attempt budget is exhausted on structural mismatch', async () => {
    ...
    sendMock.mockResolvedValue(
      makeBedrockResponse(JSON.stringify([{ number: 999, concept: 'wrong' }])),
    );
  ...
  it('hard-fails on malformed JSON output after the attempt budget is exhausted', async () => {
    ...
    sendMock.mockResolvedValue(makeBedrockResponse('this is not JSON'));
