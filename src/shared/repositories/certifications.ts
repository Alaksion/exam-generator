...
export async function getCertificationByProviderCode(
  ...
  return (result.Items?.[0] as Certification) ?? null;
}
...
export async function createCertification(certification: Certification): Promise<void> {
  await client.send(
    new PutCommand({
      TableName: config.certificationsTable,
      Item: certification,
...
export async function updateCertification(
  id: string,
  updates: Partial<Omit<Certification, 'id' | 'provider' | 'code'>>,
): Promise<void> {
